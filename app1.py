...
@st.cache_data
def load_reference_features(train_folder):
    features = {}
...
# Matcher
def match_image(test_image, reference_features, train_folder):
...
# Define path to training folder
TRAIN_DIR = Path(__file__).parent / "Training"
reference_features = load_reference_features(str(TRAIN_DIR))

uploaded_file = st.file_uploader("Upload or take a fabric image:", type=["jpg", "png"])
...
if uploaded_file:
    ...
    with st.spinner("Matching pattern..."):
        pattern, similarity, matched_img = match_image(image, reference_features, str(TRAIN_DIR))
